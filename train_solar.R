...
# Special handling of tmp values  
                 if (!tmp) {           
                    predictors[j,ii+col]<-ncvar_pred2[j,k,l,m]
                    col<-col+1
                 } else {
                    for(n in 1:5) {
                        predictors[j,ii+col]<-ncvar_pred2[j,k,l,m,n]
                        col<-col+1
                    }
                 }
...
save(predictors,file=paste(path,"solar_energy_predictors_1.RData"))

RF<-randomForest(predictors,solar_energy$ACME[1:ticks],ntree=ticks*10,importance=TRUE,localImp=TRUE)
save(RF,file="~/solar/RF.RData")
mae<-sum(abs(RF$predicted-solar_energy$ACME[1:ticks]))/ticks
mae
